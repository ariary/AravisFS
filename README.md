...
- for `adret decrypt cat` we could not `cat` big file. *It is due to the fact that we take the encrypted content to show from command-lin. Hence we are limited by `ARG_MAX` length (`getconf ARG_MAX`to know the value). For such reason avoid embedding binary (or try to compress it using `upx` command before). This limitation apply for all command in fact*
